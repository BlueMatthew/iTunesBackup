...
int MsgBoxTimeout(HWND hWnd, UINT uStrId, DWORD dwMilliseconds, UINT uType/* = MB_OK*/)
{
	CString text;
	text.LoadString(uStrId);

	return MsgBoxTimeout(hWnd, text, dwMilliseconds, uType);
}

int MsgBoxTimeout(HWND hWnd, const CString& text, DWORD dwMilliseconds, UINT uType/* = MB_OK*/)
{
	CString caption;
	caption.LoadString(IDR_MAINFRAME);
	return ::MessageBoxTimeout(hWnd, (LPCTSTR)text, (LPCTSTR)caption, uType, 0, dwMilliseconds);
}
